from java.java_structures import JavaClass, JavaMethod, JavaField, JavaAnnotation, JavaArg
...
class JavaClassVisitor(JavaParserVisitor):
    def __init__(self, file_path: str, java_classes: dict[str, JavaClass]):
        ...
        self.importsList = []
        self.activeClasses = Queue(JavaClass)
    ...
    def visitImportDeclaration(self, ctx:JavaParser.ImportDeclarationContext):
        ...
        if ctx.getChild(last - 1).getText() == "*" and ctx.getChild(last - 1).getText() == ".":
            qualified_name += "."
        self.importsList.append(qualified_name)
        return None
    ...
    def visitAnnotation(self, ctx:JavaParser.AnnotationContext):
        ...
        return JavaAnnotation(qualified_name, values)
    ...
    def visitClassDeclaration(self, ctx:JavaParser.ClassDeclarationContext):
        ...
        cls.interfaces = implementations
        self.activeClasses.push(cls)
        self.visit(ctx.classBody())
        java_class = self.activeClasses.pop()
        qualified_name = f"{java_class.package}.{java_class.name}" if java_class.package else java_class.name
        self.java_classes[qualified_name] = java_class
        return None
        ...
        if ctx.formalParameters().formalParameterList() is not None:
            for paramCtx in ctx.formalParameters().formalParameterList().formalParameter():
                ...
                args.append(JavaArg(
                    ...
                    type_=self.visit(paramCtx.typeType()),
            ...
            if ctx.formalParameters().formalParameterList().lastFormalParameter() is not None:
                ...
                is_final = False
                annotations = []
                for varMod in lfp_ctx.variableModifier():
                    # variableModifier: FINAL | annotation
                    if varMod.FINAL():
                        is_final = True
                    else:
                ...
                args.append(JavaArg(
                    is_final=is_final,
                    annotations=annotations,
                    type_=self.visit(lfp_ctx.typeType()),
                    name=lfp_ctx.variableDeclaratorId().IDENTIFIER().getText()
                ))
        line = ctx.start.line
        self.activeClasses.peek().add_method(
    ...
    def visitFieldDeclaration(self, ctx:JavaParser.FieldDeclarationContext):
        ...
        for varCtx in ctx.variableDeclarators().variableDeclarator():
            ...
            field_type = self.visit(ctx.typeType())
            idx = varCtx.variableDeclaratorId().getText().find("[")
            if idx > 0:
                field_type += varCtx.variableDeclaratorId().getText()[idx:]
            self.activeClasses.peek().add_field(JavaField(self.lastModifier, self.isStatic, field_type, name))
